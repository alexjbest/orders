...
So although we can certainly find some orders with this method we cannot hope to find all of them in general using these techniques.
...
Now for $g$ we note that $Z + I$ is a $Z$-algebra as it is a ring invariant under multiplication by elements of $Z$. % TODO more deets?
...
An equivalent definition for the conductor of an order $R$ in another $S$ is that it is the largest ideal of $S$ that is contained in $R$. %TODO check
...
{
Factor the ideal $p\ZZ_K$ into prime ideals $\p_i$, store them by norm, also add the $p$-adic order of the norm of each $\p_i$ to a set $S$.\\
...
%TODO orders represented by matrices
...
\begin{alg}\label{alg:hasone}~\\ %TODO check
...
\eIf{$H^{-1}\mathbf v$ is an integer matrix}
{
Return yes.
}
{
...
\begin{defn}[$n$-division field] %TODO find ref for this
...
So the $n$-division field is always a number field. %TODO ref
...
%as $\QQ(E[2])$ is the splitting field of $f$ and $f$ is irreducible. %TODO checkeroo
...
\disc(f) = 2^8 \Delta_E,
\]
this can be seen from the definition of $\Delta_E$. %TODO can it?
...
While the polynomial does change if a different ring generator is used (i.e using some $\alpha + n$ instead of $\alpha$, with $n\in\ZZ$) the different polynomials will give isomorphic curves.%TODO check???
