...
% - Be consistent with \colon and \mid, use mid too many colons already. How to do index?? : or \;\colon?
...
\usepackage{amsmath, amssymb, amsfonts, amsthm, hyperref, listings, tikz, algorithm2e}
...
We explain algorithms for performing computations within algebraic number theory.
...
As with the above sections everything here can be found in textbooks on elliptic curves (such as \cite{knapp}), we just move quickly to the parts relevent to us in this section.
%TODO more refs
...
This plot of the real points is typical for an elliptic curves though there can easily be two connected components.
\begin{figure}
\centering
...
Find the submodule $M$ of $\mathcal O$ corresponding to $H$.\\
\If{$M$ is a subring}
{
add $M$ to the output.
}
